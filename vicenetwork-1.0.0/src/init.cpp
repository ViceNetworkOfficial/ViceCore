...
#include "config/vicenetwork-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("vicenetwork-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "vicenetwork.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "vicenetworkd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the VN money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in VN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in VN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, vicenetwork, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in VN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all VICE specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/VICE-Ecosystem/VICE-Core>";
    const std::string URL_WEBSITE = "<https://vicenetwork.io>";

    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Dash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The ZENZO Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The VICE Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           strprintf(_("Please contribute if you find VICE Core useful. "
...
{
    RenameThread("vicenetwork-loadblk");
...
/** Sanity checks
 *  Ensure that VICE Core is running in a usable environment with all
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZVNMinted();
                        RecalculateZVNSpent();
                    }
                    RecalculateVNSupply(1);
                }
                ...
                // VICE: recalculate Accumulator Checkpoints that failed to database properly
