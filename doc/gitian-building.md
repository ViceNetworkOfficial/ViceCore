...
- [Building VICE](#building-vicenetwork)
...
Clone the git repositories for vicenetwork and gitian and then checkout the vicenetwork version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/eastcoastcrypto/ViceNetwork.git
cd VICE-Core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vicenetwork repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building ViceNetwork
----------------

To build ViceNetwork (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vicenetwork repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vicenetwork/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/ViceNetwork
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/ViceNetwork.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit vicenetwork=${COMMIT} --url vicenetwork=${URL} ../ViceNetwork/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vicenetwork=${COMMIT} --url vicenetwork=${URL} ../ViceNetwork/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vicenetwork=${COMMIT} --url vicenetwork=${URL} ../ViceNetwork/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vicenetwork-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vicenetwork-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vicenetwork-build.assert
```
